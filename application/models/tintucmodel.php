...
class tintucmodel extends CI_Model {
	...
	function getTinLienQuan($idstring){
		$query = $this->db->query("select * from tbl_tin_tuc where hide=0 and id NOT IN ".$idstring);
		return $query -> result_array();
	}
	...
	function getHome(){
		$query = $this->db->query("select * from tbl_tin_tuc where hide=0 and trang_chu=1");
		return $query -> result_array();
	}
}