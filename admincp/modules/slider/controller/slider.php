...
class slider extends Module {
	...
	function index() {
		...
		if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
			...
			for($i = 0; $i < 5 ; $i ++){
				...
				if ($imga != "") {
					...
					$row = $this->row ( 'select * from #__slides where id = "' . $j . '"' );
					$img = $row ['hinh'];
		...
		$sql = "select * from #__slides order by id asc";
	...
	// publish
	function remove() {
		$row = $this->row ( 'select * from #__slides where id = "' . $_GET ['id'] . '"' );
		$img = $row ['hinh'];
		unlink ( "../application/static/upload/images/slider/" . $img );
		unlink ( "../application/static/upload/images/slider/small_" . $img );
		
		$this->query ( "Update #__slides Set hinh='', date_create=".time()." Where id=" . $_GET ['id'] );
		$this->redirect ( 'index.php?p=' . $_GET ['p'] );
	}
}
